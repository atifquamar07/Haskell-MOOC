...
data Country = Finland | Switzerland | Norway deriving Show
...
data List a = Empty | LNode a (List a) deriving Show
...
data Egg = ChickenEgg | ChocolateEgg deriving Show
...
data RationalNumber = RationalNumber Integer Integer deriving Show
...
simplify (RationalNumber num denom) = 
  let divisor = gcd num denom
  in RationalNumber (num `div` divisor) (denom `div` divisor)
