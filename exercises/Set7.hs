...
data Distance = Distance Double deriving (Show,Eq)
data Time = Time Double deriving (Show,Eq)
data Velocity = Velocity Double deriving (Show,Eq)
...
data Event = AddEggs | AddFlour | AddSugar | Mix | Bake deriving (Eq,Show)
data State = Start | EggAdded | FlourAdded | SugarAdded | Mixed | Baked | Error | Finished deriving (Eq,Show)
...
step FlourAdded AddSugar = Mixed
step SugarAdded AddFlour = Mixed
step Mixed Mix = Baked
step Baked Bake = Finished
step _ _ = Error

bake :: [Event] -> State
bake events = go Start events
  where go state [] = if state == Baked then Finished else Error
...
data Operation1 = Add1 Int Int | Subtract1 Int Int | Multiply1 Int Int deriving Show
...
data PasswordRequirement =
  ...
  | And PasswordRequirement PasswordRequirement -- and'ing two
  | Or PasswordRequirement PasswordRequirement  -- or'ing
  deriving Show

passwordAllowed :: String -> PasswordRequirement -> Bool
passwordAllowed pwd (MinimumLength len) = length pwd >= len
passwordAllowed pwd (ContainsSome chars) = any (`elem` chars) pwd
passwordAllowed pwd (DoesNotContain chars) = not (any (`elem` chars) pwd)
