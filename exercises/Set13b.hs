...
ifM opBool opThen opElse = opBool >>= \cond -> if cond then opThen else opElse
...
path maze place1 place2 = place2 `elem` visitedPlaces
  where
    (_, visitedPlaces) = runState (visit maze place1) []
