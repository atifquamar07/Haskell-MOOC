...
prettyPrint size coords = unlines [ [if (r, c) `elem` coords then 'Q' else '.' | c <- [1..size]] | r <- [1..size]]
...
danger candidate = any (\queen -> sameRow candidate queen || sameCol candidate queen || sameDiag candidate queen || sameAntidiag candidate queen)
...
prettyPrint2 size queens = unlines [ [cell (r, c) | c <- [1..size]] | r <- [1..size]]
  where
    cell coord
      | coord `elem` queens = 'Q'
      | any (flip danger queens) [coord] = '#'  -- Correctly check if coord is in danger from any queens
      | otherwise = '.'
...
fixFirst size ((r,c):qs) = listToMaybe [((r,c'):qs) | c' <- [c..size], not (danger (r,c') qs)]
...
continue [] = [(1, 1)]  -- Initial case if the stack is empty
continue s@((r, _):_) = nextRow (r, 0) : s
...
step size stack = maybe (backtrack stack) continue (fixFirst size stack)
